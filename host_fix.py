...
print("Hole saubere Originaldateien...")
...
with open("routes/sessions.py", "w", encoding="utf-8") as f:
    f.write(rs)

print("Kopiere Dateien in den Container...")
subprocess.run(["docker", "cp", "database.py", "vbrowser-backend:/app/database.py"])
subprocess.run(["docker", "cp", "session_manager.py", "vbrowser-backend:/app/session_manager.py"])
subprocess.run(["docker", "cp", "routes/sessions.py", "vbrowser-backend:/app/routes/sessions.py"])

print("Starte Container neu...")
subprocess.run(["docker", "restart", "vbrowser-backend"])
